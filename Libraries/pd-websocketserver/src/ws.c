...
int clock_gettime(int t, timespec* spec)
{
...
static void *ws_accept(t_websocketserver *x)
{
	...
	{
		/* Accept. */
		logpost(x,2,"New client socket accepted (1)");
		new_sock =
			accept(x->sock, NULL, NULL);
		logpost(x,2,"New socket accepted  (1)");
		if (new_sock < 0)
		{
			close_socket(new_sock);
			logpost(x,2,"Closing client socket");
			break;
		}
...
//		panic("Cannot allocate accept data, out of memory!\n");
	...
	logpost(x,2,"websocket port: %d waiting for incoming connections (damn!)...", x->pd_port);
	SETSYMBOL(&x->serverstatus[0], gensym("server-running"));
	...
	/* Accept connections. */
	if (!thread_loop) {
		logpost(x,2,"Accepting connections...");
		ws_accept(x);
	}
	else 
	{
